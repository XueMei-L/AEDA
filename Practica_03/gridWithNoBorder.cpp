...
GridWithNoBorder::GridWithNoBorder(int rows, int cols) :_rows(rows), _cols(cols){
...
// void GridWithNoBorder::insertCellStatus();    // poner celulas vivas

Cell& GridWithNoBorder::getCell(int i, int j) {
    getMalla()[i]->data()[j];
    // ptr_grid->getState();
}

const Cell& GridWithNoBorder::getCell(int i, int j) const {
    getMalla()[i]->data()[j];
}

void GridWithNoBorder::insertCellStatus() {
    int stateAliveI;
    int stateAliveJ;
    int stopInsert = 1;
...
// void GridWithNoBorder::nextGeneration() {
//     // Counter neighbors
